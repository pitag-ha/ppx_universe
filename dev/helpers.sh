...
pull () {
  ...
  do
    ...
    tmp=${tmp#\"}
    DEV_REPO=${tmp#git+}
    git submodule add $DEV_REPO $basename || echo "couldn't add $basename: $DEV_REPO" >> do_manually.txt
    case $1 in
      janestreet|js)
        # To checkout to the latest released version
        (cd $basename &&
        (git checkout $ver || git checkout v$ver || echo "couldn't checkout $basename to $ver or v$ver" >> do_manually.txt) &&
